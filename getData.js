...
async function getVideoDurations(videoArray, counter, error = false) {
    ...
    async function indivVideo () {
        ...
        let priorDuration = videoDurations[aweme_id] && videoDurations[aweme_id][numericalYesterday] && videoDurations[aweme_id][numericalYesterday].durationAtDate ? videoDurations[aweme_id][numericalYesterday].durationAtDate : 0
        let todayDuration = result['video_total_duration']['value'] || 0
        let priorViews = videoDurations[aweme_id] && videoDurations[aweme_id][numericalYesterday] && videoDurations[aweme_id][numericalYesterday].viewsAtDate ? videoDurations[aweme_id][numericalYesterday].viewsAtDate : 0
