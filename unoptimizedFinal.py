...
#        font.LoadFont("../../../fonts/7x13.bdf")
	...
	ypos = 13
	    ...
	    days, remainder = divmod(time_val, 86400)
	    hours, remainder = divmod(remainder, 3600)
	    minutes, seconds = divmod(remainder, 60)

	    print(time_val, days, hours, minutes, seconds, "<- from gettimeunit")
...
#            print("distance yday timeval", time_val)
            d = np.floor(time_val / (60 * 60 * 24))
            d_in_ms = d * 86400
 #           print("p", (time_val / (60 * 60 * 24)))
  #          print("d", d)
            h = np.floor((time_val - d_in_ms) / (60 * 60))
            h_in_ms = h * (86400/24)

            m = np.floor((time_val - (d_in_ms + h_in_ms)) / (60))
            m_in_ms = m * (60)

            s = np.floor((time_val - (d_in_ms + h_in_ms + m_in_ms)))
...
##################### FIX THIS YESTERDAY LINE BACK TO CURRENT DATE PLS
        print("get duration of today and day before")
#        with open('../../../../clock_production/data/' + yesterday_date + '.json') as f:
 #           data = json.load(f)
...
#        unix_epoch_s = math.trunc(datetime.datetime.now().timestamp())
        ...
        unix_epoch_ms = unix_epoch_s
        day_length_s = 60 * 60 * 24
        day_length_ms = day_length_s
        ...
        birthdate_in_ms = 829298700

	print("birthdate_ms", birthdate_in_ms)
        ...
        ######################################################################
        ######################################################################
	print("viewTimeToday:", viewTimeToday)
	print("viewTimeYeday:", viewTimeYesterday)
	print("past test cases")
        distanceToday = viewTimeToday - timeAliveToday
	
        distanceYesterday = viewTimeYesterday - timeAliveYesterday
	print("distanceTday:", distanceToday)
	print("distanceYday:", distanceYesterday)
	print("distanceMove:", distanceToday - distanceYesterday)
	...
	print(distanceMovement, "<- distancemovement")
        numIncrements = distanceMovement

        # stop any decimal funny business again 
        if numIncrements > -1 and numIncrements < 1:
	    print("numIncrements is between -1 and 1")
            numIncrements = 1

	print("numIncrements", numIncrements)
        interval = abs((60 * 60 * 24) / numIncrements)
	
        msInterval = interval
	print("calculat:", (60*60*24) / numIncrements)
...
#            my_text = str(d) +  ':' + str(h) +  ':' + str(m) + ':' + s
            my_text = "{:03d}:{:02d}:{:02d}:{:02d}".format(d, h, m, s)
            print(color, d, ':', h, ':', m, ':', s)
	    print(my_text, "my_text")

            # original python stuff:
            offscreen_canvas.Clear()
            len = graphics.DrawText(offscreen_canvas, font, pos, ypos, color, my_text)
